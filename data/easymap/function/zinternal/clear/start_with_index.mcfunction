...
execute if score #length easymap matches 0 run return run tellraw @a[tag=is_admin] {"text":"?? idfk","color":"gray"}
$data modify storage easymap:data args set from storage easymap:data history_data[$(i)]

execute unless data storage easymap:data args.dimension run return run tellraw @a[tag=is_admin] {"text":"[ERROR] No dimension data provided",color:"red"}
function easymap:zinternal/spawn_dimension_tracker with storage easymap:data args

function easymap:zinternal/clear/start
$data remove storage easymap:data history_data[$(i)]